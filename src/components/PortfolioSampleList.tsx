...
function SampleArticleLink({ article }: { article: SampleArticle }) {
  ...
  return (
    <Link
      href={article.href}
      className="portfolio-sample-link group flex gap-3 py-5 sm:gap-4 sm:py-6 md:gap-6"
      target={isExternal ? "_blank" : undefined}
      rel={isExternal ? "noopener noreferrer" : undefined}
    >
      <div className="relative h-16 w-24 shrink-0 overflow-hidden rounded-md bg-slate-100 shadow-sm ring-1 ring-slate-200/80 sm:h-20 sm:w-28 md:h-24 md:w-36">
        <Image
          src={article.image}
          alt={article.title}
          fill
...
export function PortfolioSampleList() {
  return (
    <div className="mx-auto flex w-full flex-col items-center">
      <div className="inline-flex w-full max-w-full flex-col md:ml-8">
        ...
        <Image
          ...
          width={1024}
          height={583}
          className="portfolio-proof-image mx-auto mt-6 block w-full max-w-[60rem]"
          unoptimized
        />
        <Image
